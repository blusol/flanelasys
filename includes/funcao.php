...
/*  Função para remover a acentuação e caracteres especiais de uma string
 *   Parâmetros: $str -> String a ser verificada
  $caracter -> caracter  usado para substituir
  (senão for especificado substitui pela letra/caracter equivalente Ex. Ã = A, Ç = C)
...
function remove_acentuacao($str, $caracter = "") {
    ...
    if ($caracter == "") {
        ...
        $str = preg_replace('/&((?i)[a-z]{1,2})(?:grave|accent|acute|circ|tilde|uml|ring|lig|cedil|slash);/', $caracter, $str);
        $str = str_replace(array('&ETH;', '&eth;', '&THORN;', '&thorn;'), array($caracter, $caracter, $caracter, $caracter), $str);
    }
    return $str;
}
